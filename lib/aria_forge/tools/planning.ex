...
defmodule AriaForge.Tools.Planning do
  ...
  - `batch: true` - Enable batch mode to group independent steps together
  """
  @spec execute_plan(map() | String.t(), String.t(), keyword()) :: planning_result()
  def execute_plan(plan_data, temp_dir, opts \\ []) do
    plan = 
      case plan_data do
        plan when is_map(plan) -> plan
        plan_string when is_binary(plan_string) ->
          case Jason.decode(plan_string) do
            {:ok, decoded_plan} -> decoded_plan
            error -> {:error, "Failed to decode plan: #{inspect(error)}"}
          end
      end
    ...
    case plan do
      ...
      plan_map when is_map(plan_map) ->
        Planning.Execution.execute_plan_steps(plan_map, temp_dir, opts)
    end
  end

  # Private helper functions (execution moved to Planning.Execution module)
