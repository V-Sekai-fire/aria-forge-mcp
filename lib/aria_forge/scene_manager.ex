...
defmodule AriaForge.SceneManager do
  @moduledoc """
  GenServer that manages a single scene context.

  Provides process-based isolation and barriers for operations.
  ...
  def init(scene_id) do
    ...
    temp_dir = AriaForge.NativeService.Context.create_temp_dir()
  ...
  def handle_call({:create_cube, name, location, size}, _from, state) do
    result = AriaForge.Tools.create_cube(name, location, size)
  ...
  def handle_call({:create_sphere, name, location, radius}, _from, state) do
    result = AriaForge.Tools.create_sphere(name, location, radius)
  ...
  def handle_call({:set_material, object_name, material_name, color}, _from, state) do
    result = AriaForge.Tools.set_material(object_name, material_name, color)
  ...
  def handle_call({:render_image, filepath, resolution_x, resolution_y}, _from, state) do
    result = AriaForge.Tools.render_image(filepath, resolution_x, resolution_y)
  ...
  def handle_call(:get_scene_info, _from, state) do
    ...
    result = AriaForge.Tools.get_scene_info(temp_dir)
  ...
  defp via_tuple(scene_id) do
    {:via, Registry, {AriaForge.SceneRegistry, scene_id}}
  end
